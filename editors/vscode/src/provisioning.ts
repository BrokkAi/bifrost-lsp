...
const OWNER = "BrokkAi";
const REPO = "bifrost-lsp";
const BINARY_NAME = "bifrost-lsp";
...
export async function findCompatibleManagedBinary(
  ...
  const exactIndex = compatibleVersions.indexOf(normalized.serverVersion);
  if (exactIndex > 0) {
    compatibleVersions.splice(exactIndex, 1);
    compatibleVersions.unshift(normalized.serverVersion);
  }
  ...
  for (const version of compatibleVersions) {
    ...
    try {
      ...
      return {
        path: candidate,
        version,
        compatibilityMode: version === normalized.serverVersion ? "exact" : "compatible"
      };
    } catch {
      continue;
    }
...
export function parseBifrostVersion(output: string): string | null {
  const match = /\bbifrost(?:-lsp)?\s+v?([0-9]+(?:\.[0-9]+){1,2}(?:[-+][^\s]+)?)/.exec(output);
  return match?.[1] ?? null;
}
...
export function isVersionCompatible(
  ...
  try {
    ...
    const preferred = parseVersion(normalized.serverVersion, "serverVersion");
    const minimum = parseVersion(normalized.minimumServerVersion, "minimumServerVersion");
    return (
...
export function normalizeBinaryCompatibility(
  ...
  const serverVersion = normalizeVersion(compatibility.serverVersion);
  const minimumServerVersion = normalizeVersion(compatibility.minimumServerVersion);
  const preferred = parseVersion(serverVersion, "serverVersion");
  const minimum = parseVersion(minimumServerVersion, "minimumServerVersion");
  ...
  if (preferred.major !== minimum.major || preferred.minor !== minimum.minor) {
    throw new Error(
      `Bifrost minimumServerVersion ${minimumServerVersion} must use the preferred ${preferred.major}.${preferred.minor} minor series`
    );
  }
  if (compareParsedVersions(minimum, preferred) > 0) {
    throw new Error(
      `Bifrost minimumServerVersion ${minimumServerVersion} cannot exceed serverVersion ${serverVersion}`
    );
  }
  return { serverVersion, minimumServerVersion, allowPrerelease: compatibility.allowPrerelease };
}
