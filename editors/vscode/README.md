...
extracts the `bifrost-lsp` executable into VS Code global storage at
...
- `package.json` has `bifrost.serverVersion` set to the standalone server release version
  without the leading `v`.
- `package.json` has a compatible `bifrost.minimumServerVersion` and explicit
  `bifrost.allowPrerelease` policy.
...
- The matching GitHub Release contains archive assets named
  `bifrost-lsp-v<version>-<target>.tar.gz` on macOS/Linux and
  `bifrost-lsp-v<version>-<target>.zip` on Windows.
