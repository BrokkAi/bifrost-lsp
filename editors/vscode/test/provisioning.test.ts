...
void test("constructs release archive names and URLs", () => {
  ...
  assert.equal(asset.archiveName, "bifrost-lsp-v0.6.8-x86_64-unknown-linux-gnu.tar.gz");
  assert.equal(asset.checksumName, "bifrost-lsp-v0.6.8-x86_64-unknown-linux-gnu.tar.gz.sha256");
  assert.equal(
    asset.archiveUrl,
    "https://github.com/BrokkAi/bifrost-lsp/releases/download/v0.6.8/bifrost-lsp-v0.6.8-x86_64-unknown-linux-gnu.tar.gz"
  );

  const windows = provisioning.releaseAssetFor("v0.6.8", "win32", "arm64");
  assert.equal(windows.archiveName, "bifrost-lsp-v0.6.8-aarch64-pc-windows-msvc.zip");
  assert.equal(windows.checksumName, "bifrost-lsp-v0.6.8-aarch64-pc-windows-msvc.zip.sha256");
});
...
void test("parses and validates SHA-256 sidecars", () => {
  ...
  assert.equal(
    provisioning.parseSha256(
      `${hash}  bifrost-lsp-v0.6.8-target.tar.gz\n`,
...
void test("installs verified binary and preserves compatible managed versions", async () => {
  ...
  fs.writeFileSync(path.join(oldDir, "bifrost-lsp"), "old");
  const incompatibleDir = path.join(temp, "binaries", "0.5.9", "linux-x64");
  fs.mkdirSync(incompatibleDir, { recursive: true });
  fs.writeFileSync(path.join(incompatibleDir, "bifrost-lsp"), "incompatible");
  ...
  fs.writeFileSync(path.join(stageDir, "bifrost-lsp"), "new-binary");
  ...
  const preparation = await provisioning.selectManagedBinaryAndPreparePreferred(
    () =>
      provisioning.findCompatibleManagedBinary(
        temp,
        { serverVersion: "0.9.4", minimumServerVersion: "0.9.0", allowPrerelease: false },
        "linux",
        "x64",
...
void test("skips mislabeled, prerelease, and cross-minor managed binaries", async () => {
  ...
  const selected = await provisioning.findCompatibleManagedBinary(
    temp,
    { serverVersion: "0.9.5", minimumServerVersion: "0.9.0", allowPrerelease: false },
    "linux",
    "x64",
    (binary) => {
...
void test("rejects checksum mismatch during install", async () => {
  ...
  const fetchImpl: typeof fetch = (url) => {
    if (requestUrl(url).endsWith(".sha256")) {
      return Promise.resolve(
        new Response(`${"b".repeat(64)}  bifrost-lsp-v0.6.8-x86_64-unknown-linux-gnu.tar.gz\n`)
      );
    }
...
void test("rejects archive bytes that do not match the pinned checksum", async () => {
  ...
  const fetchImpl: typeof fetch = (url) => {
    if (requestUrl(url).endsWith(".sha256")) {
      return Promise.resolve(
        new Response(`${expectedSha256}  bifrost-lsp-v0.6.8-x86_64-unknown-linux-gnu.tar.gz\n`)
      );
    }
    return Promise.resolve(new Response(Buffer.from("not-a-real-archive")));
  };
...
void test("builds managed launch config when bundled mode has an installed binary", () => {
  ...
  assert.deepEqual(config.args, ["--root", "/workspace", "--flag"]);
